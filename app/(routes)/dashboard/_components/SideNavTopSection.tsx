...
function SideNavTopSection({user,setActiveTeamInfo}:any) {
  ...
  return (
      ...
      <PopoverContent className="ml-7 p-4">
        ...
        <div>
            {TeamList?.map((team,index)=>(
                <h2 key={index} className={`p-2 hover:bg-blue-500 hover:text-white rounded-lg mb-1 cursor-pointer ${activeTeam?._id==team._id&&'bg-blue-500 text-white'}`} onClick={()=>setActiveTeam(team)}>
                    {team.teamName}</h2>
            ))}
        </div>
        ...
        <div>
            {menu.map((item,index)=>(
                <h2 key={index} className='flex gap-2 items-center p-2 hover:bg-gray-100 rounded-lg text-sm cursor-pointer' onClick={()=>onMenuClick(item)}>
    ...
    {/* All File Button */}
    <Button variant='outline' className='w-full justify-start gap-2 font-bold mt-8 bg-gray-2100'>
